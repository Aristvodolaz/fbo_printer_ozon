...
Сервер запустится на `http://localhost:22`

**⚠️ Внимание:** Порт 22 обычно используется для SSH. Если возникнет конфликт, используйте другой порт через переменную окружения: `PORT=3022 npm start`
